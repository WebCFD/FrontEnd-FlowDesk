...
- June 29, 2025. PERFORMANCE FIX: Resolved Canvas3D infinite loop during AirEntry dialog initialization. Root cause was using IIFE (Immediately Invoked Function Expression) directly in initialValues prop, causing re-execution on every Canvas3D render. Solution: Replaced IIFE with useMemo hook and editingAirEntry dependency to memoize initialValues calculation. Loop eliminated while preserving all functionality - dialog still shows correct wallPosition values and synchronization works perfectly. System now performs efficiently without redundant computations.