...
- July 8, 2025. CANVAS3D SAVE CHANGES POSITION PERSISTENCE BUG RESOLUTION COMPLETE: Fixed critical issue where Canvas3D AirEntry dialogs were not positioning windows correctly during Save Changes operations when no modifications were made, while Canvas2D worked correctly. Root cause identified: Canvas2D preserved current store position during Save Changes using `currentStorePosition = updatedAirEntries[index]?.position`, while Canvas3D used original `editingAirEntry.entry.position` which could be stale. Solution implemented: Modified Canvas3D handleAirEntryEdit to match Canvas2D pattern by reading current position from store using `useRoomStore.getState().floors[currentFloor]?.airEntries?[index]?.position` and preserving it in updatedEntry. Both Canvas2D and Canvas3D now use identical position preservation strategy during Save Changes operations, ensuring consistent behavior across all views.
- July 8, 2025. CRITICAL FLOOR LOAD REFERENCE SHARING BUG RESOLUTION COMPLETE: Fixed multiple critical issues with Floor Load functionality where copied AirEntry elements were linked to their originals and positioned incorrectly when switching floors. Root cause analysis: (1) `regenerateAirEntryIds` performed shallow cloning with `{ ...entry.properties }` causing shared references between original and copied elements, (2) `updatedAirEntryPositionsRef` cache used normalized floor names causing position conflicts between floors, (3) `handleUpdateAirEntryFrom3D` was reading from current floor's `airEntries[index]` instead of target floor data, causing cross-floor interference when elements had identical indices, (4) Real-time update handlers in Canvas2D/Canvas3D used array indices which could match between different floors after Load operations. Solution implemented: (1) Replaced shallow cloning with deep cloning using `JSON.parse(JSON.stringify())` for properties, dimensions, and line references in `regenerateAirEntryIds`, (2) Enhanced `clearCanvas3DPositionCache` function to clear specific element IDs instead of entire floors, (3) Fixed `handleUpdateAirEntryFrom3D` to read from target floor data (`floors[floorName]?.airEntries[index]`) instead of current floor data, (4) Modified Canvas2D real-time handlers to use element IDs (`handleAirEntryPositionUpdate(elementId, ...)`) instead of indices, (5) Updated Canvas3D position cache to use element IDs as keys, (6) Integrated targeted cache clearing into `performFloorLoad` workflow. AirEntry elements copied with Load now have completely independent properties and positioning behavior without cross-floor interference. Position modifications are now element-specific and do not affect other elements regardless of floor or index position.