...
- July 8, 2025. CANVAS3D CROSS-FLOOR INTERACTIVITY ENHANCEMENT COMPLETE: Removed currentFloor restriction from AirEntry access in Canvas3D view to enable interaction with elements from all floors simultaneously. Modified handleAirEntryDoubleClick function to search through ALL floors instead of only the selected currentFloor. Implementation includes: (1) Priority search using mesh userData.floorName when available, (2) Fallback search across all floors using exact and approximate position matching, (3) Proper floor context tracking for dialog initialization, (4) Maintained all existing functionality for dimensions updates and wall context. Users can now double-click and edit AirEntry elements from any floor in 3D view regardless of currentFloor selection, providing more intuitive multi-floor building design workflow while preserving data integrity and synchronization.