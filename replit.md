...
- July 11, 2025. CROSS-FLOOR VISUAL GLITCH RESOLUTION COMPLETE: Successfully identified and resolved the root cause of visual contamination where moving AirEntry on one floor caused momentary movement on other floors. Through comprehensive debugging logs, discovered the issue was NOT shared object references as initially suspected, but rather Canvas3D completely rebuilding the entire scene during every real-time update. Problem was in finalFloors useMemo dependency array using [floors, storeFloors] which triggered scene reconstruction on every position change. Solution implemented: Modified dependency array to only track structural changes (lineCount, airEntryCount, furnitureCount, stairCount, hasClosedContour) instead of full storeFloors data. This prevents unnecessary scene rebuilds during real-time updates while preserving rebuilds for actual structural changes. Visual glitch completely eliminated - AirEntry real-time modifications now work smoothly without cross-floor contamination.