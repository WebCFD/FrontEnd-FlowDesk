...
- June 29, 2025. ADDED CANVAS2D SAFETY CHECKS FOR DIMENSIONS: Implemented comprehensive safety checks throughout Canvas2D.tsx for entry.dimensions access due to useMemo exclusion optimization. Added fallback dimensions ({ width: 100, height: 100 }) in drawAirEntry, findAirEntryAtLocation, AirEntryDialog initialValues, and real-time update functions. These checks prevent runtime errors when dimensions are undefined due to performance optimization exclusions while maintaining visual functionality with sensible defaults.