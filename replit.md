...
- June 28, 2025. FINAL SOLUTION: Fixed AirEntry position reversion by correcting data propagation flow in wizard-design.tsx. Root cause was Canvas2D receiving airEntries from local state instead of floors[currentFloor].airEntries like Canvas3D. Changed Canvas2D to use floors[currentFloor]?.airEntries || airEntries, ensuring both components receive data from the same source. This enables immediate store updates to propagate correctly to Canvas2D props, eliminating the timing issue where real-time position updates weren't reflected in the final SaveChanges operation.