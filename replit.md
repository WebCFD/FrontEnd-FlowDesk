...
- July 8, 2025. CRITICAL FLOOR LOAD REFERENCE SHARING BUG RESOLUTION COMPLETE: Fixed two critical issues with Floor Load functionality where copied AirEntry elements were linked to their originals and positioned incorrectly when switching floors. Root cause analysis: (1) `regenerateAirEntryIds` performed shallow cloning with `{ ...entry.properties }` causing shared references between original and copied elements, (2) `updatedAirEntryPositionsRef` cache used normalized floor names causing position conflicts between floors. Solution implemented: (1) Replaced shallow cloning with deep cloning using `JSON.parse(JSON.stringify())` for properties, dimensions, and line references in `regenerateAirEntryIds`, (2) Added `clearCanvas3DPositionCache` function exposed via window global to clear position cache for target floor during Load operations, (3) Integrated cache clearing into `performFloorLoad` workflow. AirEntry elements copied with Load now have completely independent properties and positioning behavior without cross-floor interference.