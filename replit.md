...
- July 1, 2025. IMPLEMENTED SOLUTION 1 FOR CUSTOM OBJECT SCALE: Fixed critical bug where loaded STL objects disappeared after editing due to scale reset from {100,100,100} to {1,1,1}. Solution 1 implemented: dialog initialization now reads actual 3D object scale for custom objects instead of stored default values. Custom objects maintain necessary 100x scaling for visibility while standard furniture continues using stored scale values normally. This targeted fix preserves current architecture without disrupting data storage or export systems.
- July 1, 2025. ADDED STL FILE PATH TO JSON EXPORT: Enhanced custom furniture JSON export to include STL file location information. Updated FurnitureExport interface with optional stlFilePath field and modified export logic to extract original file name from customFurnitureStore for custom objects only. Standard furniture types (table, person, armchair, car, block, vent) remain unchanged. Custom objects now export with complete metadata including original STL filename for simulation reference and asset tracking.