...
- July 4, 2025. WALL TEMPERATURE PERSISTENCE BUG RESOLUTION COMPLETE: Fixed critical issue where wall temperature modifications showed correctly in JSON export but reverted to default values (20°C) when dialog reopened. Root cause was wall synchronization system in simulationDataConverter.ts recreating walls with defaultTemperature during syncWallsForCurrentFloor() calls, losing custom temperature values. Solution implemented: (1) Added findSimilarWallForLine() function with 1-pixel tolerance to find walls with similar coordinates during synchronization, (2) Modified addMissingWalls() function to preserve existing wall temperatures when recreating walls by checking for similar walls and using their temperature properties, (3) Wall recreation now preserves custom temperatures while maintaining all existing functionality. Wall temperature persistence now works identically to JSON export - custom temperatures are preserved through design loading, floor switching, and synchronization cycles.