...
- July 6, 2025. CRITICAL VERTICAL/HORIZONTAL ANGLE PERSISTENCE BUG RESOLUTION COMPLETE: Fixed critical issue where Vertical Angle and Horizontal Angle values were not persisting when AirEntry dialog was reopened, while Air Temperature worked correctly. Root cause identified: Two-stage bug: (1) Dialog initialization (useEffect) was loading Air Temperature from savedProps.temperature but missing initialization for verticalAngle and horizontalAngle properties, (2) Properties save function was storing Air Temperature, flowType, flowValue, flowIntensity, and airOrientation to simulationProperties but excluding verticalAngle and horizontalAngle. Solution implemented: (1) Added verticalAngle and horizontalAngle initialization in dialog useEffect for vent-specific properties loading, (2) Added simulationProperties.verticalAngle and simulationProperties.horizontalAngle to store persistence in Save Changes workflow. Vertical Angle and Horizontal Angle now have complete persistence parity with Air Temperature, maintaining values correctly through dialog reopen cycles and achieving full synchronization with real-time update architecture.
