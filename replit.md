...
- June 28, 2025. FIXED STAIR TEMPERATURE PERSISTENCE: Resolved critical bug where stair temperature modifications in StairPropertiesDialog were not persisting when dialog was reopened. Root cause was state synchronization issue between Zustand store and local component variables in wizard-design.tsx. Solution implemented direct store reading pattern: Canvas2D now accesses stairPolygons directly from currentFloorData instead of using destructured prop, ensuring real-time temperature updates persist correctly across dialog sessions.