...
- June 28, 2025. FIXED AIRENTRY SAVE CHANGES POSITION REVERT BUG: Resolved critical issue where AirEntry position changes worked correctly in real-time but reverted to initial position after clicking "Save Changes". Root cause was canvasData in onConfirm only included wallPosition percentage but not the calculated position coordinates. When Canvas2D processed the saved data, it had to recalculate position from wallPosition using initial values instead of updated ones. Solution added position field to canvasData, ensuring Save Changes preserves both wallPosition and calculated coordinates for consistent behavior.