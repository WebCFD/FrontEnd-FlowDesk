...
- July 6, 2025. CRITICAL VERTICAL/HORIZONTAL ANGLE PERSISTENCE BUG RESOLUTION COMPLETE: Fixed critical issue where Vertical Angle and Horizontal Angle values were not persisting when AirEntry dialog was reopened, while Air Temperature worked correctly. Root cause identified: Three-stage bug: (1) VentSimulationProperties interface in room-store.ts was missing verticalAngle and horizontalAngle type definitions, causing TypeScript to reject them as valid properties, (2) Dialog initialization was missing these properties due to type system exclusion, (3) Properties save function was attempting to store them but they were being filtered out by the store's type validation. Solution implemented: (1) Added verticalAngle?: number and horizontalAngle?: number to VentSimulationProperties interface in room-store.ts, (2) Removed all (as any) casting from AirEntryDialog.tsx since properties are now properly typed, (3) Ensured complete type safety for angle persistence throughout the system. Vertical Angle and Horizontal Angle now have complete persistence parity with Air Temperature, maintaining values correctly through dialog reopen cycles with full TypeScript type safety and achieving complete synchronization with real-time update architecture.
