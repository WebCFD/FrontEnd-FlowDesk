...
## CRITICAL UNRESOLVED ISSUE - July 12, 2025

**Problem**: AirEntry Cross-Window Interference and React Hook Violation
- **Status**: UNRESOLVED CRITICAL BUG
- **Symptom**: Editing one AirEntry window affects all other windows on the same floor
- **Root Cause**: Canvas3D finalFloors useMemo dependency pattern `editingAirEntry ? {} : storeFloors` creates new {} object every render, causing infinite loop (React Hook Rules violation)
- **Architecture Conflict**: The pattern that prevents cross-window interference (isolation mechanism) violates React dependency rules and causes performance issues

**Technical Analysis**:
1. **Original "Working" Pattern**: `editingAirEntry ? {} : storeFloors`
   - During editing: Uses new {} object (prevents scene rebuilds, maintains isolation)
   - After editing: Uses storeFloors (triggers complete scene rebuild with updated data)
   - **Problem**: Creates new {} object every render → infinite loop → performance degradation

2. **Failed Fix Attempts**:
   - **Attempt 1**: Used `useRef({}).current` for stable reference
   - **Result**: Eliminated infinite loop but **BROKE isolation functionality**
   - **Issue**: All windows now affected when editing one window

3. **Core Architecture Conflict**:
   - React requires stable dependency references for useMemo
   - Isolation mechanism requires **controlled instability** to prevent cross-contamination
   - These requirements are fundamentally incompatible

**Two-Phase Isolation System Understanding**:
- **Phase 1 (During editing)**: Direct mesh manipulation + stable scene (no rebuilds)
- **Phase 2 (After editing)**: Complete scene rebuild from updated store data
- **Critical Insight**: The {} object creation was **intentional instability** that achieved isolation

**Impact**:
- Application unusable for AirEntry editing
- Multiple windows move simultaneously when editing one
- User workflow completely broken
- Performance issues with infinite re-renders

**Next Steps Required**:
- Find alternative architecture that achieves isolation without React hook violations
- Consider complete refactoring of Canvas3D scene management
- Explore component isolation patterns that don't rely on useMemo tricks
- May require fundamental changes to real-time update architecture

**User Feedback**: "I modify a window, it affects all...this is a failure" - Critical functionality broken