...
- June 28, 2025. RESOLVED CRITICAL VENT PERSISTENCE BUG: Fixed fundamental issue where vents weren't persisting during export due to missing furnitureItemsLength dependency in wizard-design.tsx useMemo. Root cause was useMemo dependencies only included linesLength, airEntriesLength and hasClosedContour but excluded furnitureItems changes. Added furnitureItemsLength to dependency array, ensuring wizard state synchronizes with Zustand store when vents are added. This critical fix enables proper vent export to JSON simulation data.
- June 28, 2025. ENHANCED JSON EXPORT STRUCTURE: Completed comprehensive optimization of HVAC simulation export format. Added metadata section with export date, total floors, and multifloor status. Integrated buildingParameters section containing default ceiling height and complete floor parameter configurations (ceiling height, floor deck, ceiling/floor temperatures) with proper cm-to-meters conversion. Enhanced floor data structure includes ceiling and floor surface temperatures from Building Parameters, with vents properly reorganized as air entries in appropriate surfaces using accurate surface type detection. This creates a professional, comprehensive JSON structure suitable for advanced HVAC simulation software integration.