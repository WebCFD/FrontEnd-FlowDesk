...
### Task #17: 3D Room Geometry VTK Generation (2026-03-22)
**Problem**: VTK viewer displayed only a flat 2D horizontal slice plane. User wanted to see the full 3D room geometry (walls, floor, ceiling, doors, windows) in the web viewer.

**Root cause**: CFD field data was computed on cloud HPC and not downloaded locally. The OpenFOAM `results.foam` reader returns empty fields; only geometry (mesh topology) is available locally. Converged solution fields only exist in the 2D slice VTK files already generated by `analyze_flow_planes()`.

**Solution**:
1. **New Python function `generate_surface_3d_vtk(sim_path, post_path)`** in `objects.py`:
   - Reads all 8 OpenFOAM boundary patches (ceil, door, floor, wall×4, window) from `results.foam`
   - If CFD fields present in reader → uses them directly
   - If not (local dev case) → nearest-neighbour projection from existing `flow_plane_*.vtk` slice files using `scipy.cKDTree`
   - Saves as ASCII VTK POLYDATA to `post/vtk/surface_3d.vtk` (~3.7MB, 20,299 pts, 14 fields)
2. **New Python function `generate_volume_internal_vtk(sim_path, post_path)`** in `objects.py`:
   - Reads internalMesh from `results.foam`, calls `extract_surface()` to get PolyData
   - Same CFD field mapping logic as above
   - Saves to `post/vtk/volume_internal.vtk` (~3.7MB, 19,442 pts)
3. **Helper `_map_slice_fields_to_mesh(mesh, vtk_dir)`**: merges all `flow_plane_*.vtk` files, builds KD-tree, maps 14 fields (T, U, p, CO2, PMV, PPD, G, k, ω, etc.) to any mesh via nearest-neighbour (max dist ≈ 0.5m for ceiling).
4. **Pipeline integration** in `step05_results2post.py` steps 10 & 11 (after `analyze_flow_planes` which creates the slice files). Both calls wrapped in try/except so failures are non-fatal.
5. **Backend `server/routes.ts`**: Added `surface_3d` type detection for `surface_3d.vtk` filename. Updated sort priority: `surface_3d(0) > volume_complete(1) > boundary(2) > volume(3) > volume_internal(4) > slice(5)`. `latestVolume` now prefers `surface_3d` type — viewer loads the full 3D room by default.

**VTK format**: PyVista saves ASCII VTK 5.1 POLYDATA. First field as `SCALARS`, remaining 13 as `FIELD FieldData 13`. Compatible with existing `parseVtkAscii()` (Task #16).

**Files Modified**: `src/components/post/objects.py`, `PYTHON_STEPS/step05_results2post.py`, `server/routes.ts`

## Recent Changes (February 2026)

### Face-Based Rack JSON Export (2026-02-21)
Racks are now exported as face-based geometry instead of position/rotation/dimensions. Each rack has 6 faces (front, back, left, right, top, bottom), each defined by 4 vertices in global coordinates (meters). Inlet/outlet faces carry server properties (`rackDensity`, `thermalPower_kW`, `airFlow`), wall faces carry chassis properties (`material`, `emissivity`). The `thermalPower` field was renamed to `thermalPower_kW`. Legacy position/dimensions format for racks has been removed — all racks use the face-based format exclusively. Non-rack furniture (tables, persons, armchairs, blocks) continues to use position/dimensions format.
