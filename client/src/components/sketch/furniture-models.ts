...
export const createTableModel = (): THREE.Group => {
  ...
  // Table top
  const topGeometry = new THREE.BoxGeometry(80, 5, 60);
  ...
  top.position.y = 72.5; // Position at top of legs
  group.add(top);
  ...
  legPositions.forEach(pos => {
    ...
    leg.position.set(pos.x, 35, pos.z); // Half height of legs
    group.add(leg);
  });
...
export const createPersonModel = (): THREE.Group => {
  ...
  leftLeg.position.set(8, 25, 0);
  group.add(leftLeg);

  const rightLeg = new THREE.Mesh(legGeometry, bodyMaterial);
  rightLeg.position.set(-8, 25, 0);
  group.add(rightLeg);
...
export const createArmchairModel = (): THREE.Group => {
  ...
  // Seat
  const seatGeometry = new THREE.BoxGeometry(60, 10, 50);
  const seat = new THREE.Mesh(seatGeometry, chairMaterial);
  seat.position.y = 25;
  group.add(seat);

  // Back
  const backGeometry = new THREE.BoxGeometry(60, 50, 10);
  const back = new THREE.Mesh(backGeometry, chairMaterial);
  back.position.set(0, 45, -20);
  group.add(back);

  // Arms
  const armGeometry = new THREE.BoxGeometry(10, 30, 40);
  const leftArm = new THREE.Mesh(armGeometry, chairMaterial);
  leftArm.position.set(35, 35, 0);
  group.add(leftArm);

  const rightArm = new THREE.Mesh(armGeometry, chairMaterial);
  rightArm.position.set(-35, 35, 0);
  group.add(rightArm);
