// Placeholder component - actual implementation saved as Canvas3D.backup.tsx
...
export default function Canvas3D({
  ...
  useEffect(() => {
    ...
    const handleRightMouseDown = (event: MouseEvent) => {
      ...
      if (axesIntersects.length > 0) {
        ...
        if (axisDirection === 'x' || axisDirection === 'z') {
          // First check if we already have a selected air entry - this is the key fix
          // If so, use that instead of searching for a new one
          let closestAirEntry: THREE.Mesh | null = null;
          
          if (selectedAirEntry && selectedAirEntry.object) {
            console.log("Using already selected air entry for axis drag:", {
              index: selectedAirEntry.index,
              id: selectedAirEntry.object.id,
              type: selectedAirEntry.entry.type
            });
            closestAirEntry = selectedAirEntry.object;
          } 
          // Otherwise check if we have a parent entry index in the axis userData
          else {
        ...
        if (floorData && floorData.airEntries) {
          ...
          if (index !== -1) {
            console.log("[AIR_ENTRY_SELECTION] Air entry found:", {
              index,
              position: floorData.airEntries[index].position,
              objectId: mesh.id
            });
            
            // When selecting an air entry, update the dragStateRef to prepare for possible drag
            // operations on subsequent right-clicks
            setSelectedAirEntry({
              index: index,
              entry: floorData.airEntries[index],
              object: mesh,
            ...
            // Initialize the drag state ref with complete information for the selected object
            // This ensures we have the necessary data even if we don't start dragging immediately
            dragStateRef.current = {
              isDragging: false, // Not dragging yet, just selecting
              selectedAxis: null, // Will be set when clicking on an axis
              startPosition: mesh.position.clone(), // Save initial position in case we drag later
              initialMousePosition: {
                x: event.clientX,
                y: event.clientY
              },
              ...
              selectedObject: mesh,
              entryIndex: index,
              lastDraggedObjectId: mesh.id.toString()
            };
            
            // Highlight the selected air entry
            highlightSelectedAirEntry(mesh, true, dragStateRef);
            
            // Force a render to ensure highlight is visible
            needsRenderRef.current = true;
    ...
    const handleMouseUp = (event: MouseEvent) => {
      ...
      // On mouse up, don't reset highlighting for initial clicks - only reset if we 
      // weren't dragging AND we already had a different object selected before
      // This keeps the selection intact for consecutive drag operations
      if (dragStateRef.current && !dragStateRef.current.isDragging) {
        const isAxisOrAirEntryOperation = hoveredArrow !== null || 
            (selectedAirEntry && selectedAirEntry.object && selectedAirEntry.object.id === dragStateRef.current.selectedObject?.id);
            
        // Only reset if this wasn't part of a drag operation and it's not a selection of a new air entry
        if (!isAxisOrAirEntryOperation && dragStateRef.current.selectedObject) {
          ...
          console.log("[MAINTAINING_SELECTION] Keeping selection active for further operations");
        }
      }
      ...
      console.log("[DRAG_COMPLETE] Dragging stopped, final state:", {
        ...
        selectedAirEntry: selectedAirEntry ? {
          ...
          object: selectedAirEntry.object ? selectedAirEntry.object.id : null
        } : null,
        entryIndex: dragStateRef.current.entryIndex,
    ...
    // This effect runs once for setup and cleanup
    function handleSceneCleanup() {
      window.removeEventListener("resize", handleResize);
      if (controls) controls.dispose();
      if (renderer) {
        renderer.domElement.removeEventListener("contextmenu", e => e.preventDefault());
        renderer.domElement.removeEventListener("mousedown", handleRightMouseDown);
    ...
    return () => {
      console.log("Running cleanup function for Canvas3D");
      window.removeEventListener("resize", handleResize);
      if (controls) controls.dispose();
      if (renderer) {
        renderer.domElement.removeEventListener("contextmenu", e => e.preventDefault());
  ...
  // Empty useEffect only for initialization
  useEffect(() => {
    // Intentionally empty
  }, []);
