...
export function RoomSketchPro({
  ...
  // Function to re-apply textures (called when furniture is deleted)
  const handleFurnitureDeleted = useCallback(() => {
    console.log('RSP: Furniture deleted, re-applying textures...');
    
    // Small delay to ensure the furniture mesh is fully removed from the scene
    setTimeout(() => {
      applyThemeTextures();
    }, 100);
  }, []);

  // REMOVED: handleAirEntryUpdated - no longer needed, textures preserved automatically during direct modification
  ...
  return (
    <div 
      ref={containerRef}
      className={`relative w-full ${getThemeBackground()}`}
