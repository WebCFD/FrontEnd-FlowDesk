...
export function RoomSketchPro({
  ...
  const createMultiFloorVisualization = (scene: THREE.Scene, renderer: THREE.WebGLRenderer, camera: THREE.PerspectiveCamera) => {
    ...
    // Use shared geometry function from Canvas3D for identical representation
    const { floorsData } = resolveGeometryData();
    
    console.log("🏢 RSP: Using generateSharedFloorGeometry from Canvas3D for identical representation");
    
    if (!floorsData || Object.keys(floorsData).length <= 1) {
      console.log("RoomSketchPro - Not enough floors for multifloor visualization");
      return;
    }
    
    // Transform floors data to match Canvas3D FloorData interface
    const transformedFloorsData: Record<string, any> = {};
    Object.entries(floorsData).forEach(([floorName, floorData]) => {
      if (floorData && typeof floorData === 'object') {
        transformedFloorsData[floorName] = {
          ...floorData,
          hasClosedContour: (floorData as any).lines?.length > 2,
          name: floorName
        };
      }
    });

    // Generate identical geometry using Canvas3D's shared function
    const geometryObjects = generateSharedFloorGeometry(transformedFloorsData, {
      currentFloor: currentFloor || "ground",
      wallTransparency: wallTransparency,
      isMultifloor: true,
      defaultCeilingHeight: roomHeight || DEFAULTS.ROOM_HEIGHT,
      defaultFloorDeck: 20
    });
    
    console.log(`🏢 RSP: Generated ${geometryObjects.length} geometry objects using Canvas3D shared function`);
    
    // Create group to hold all geometry objects from Canvas3D
    const multiFloorGroup = new THREE.Group();
    multiFloorGroup.name = "multiFloorGroup";
    
    // Add all geometry objects generated by Canvas3D shared function
    geometryObjects.forEach(object => {
      multiFloorGroup.add(object);
    });
    
    // Add the complete multifloor group to the scene
    scene.add(multiFloorGroup);
    
    console.log("🏢 RSP: Successfully added shared geometry to scene with identical representation to Canvas3D");
    
    // Update camera to view all floors properly
    const allFloorNames = Object.keys(floorsData);
    if (allFloorNames.length > 1) {
      // Position camera to see all floors using similar logic to Canvas3D
      const maxHeight = allFloorNames.length * (roomHeight || DEFAULTS.ROOM_HEIGHT) + 100;
  ...
  // Function to create single floor visualization using shared Canvas3D geometry
  const createSingleFloorVisualization = (scene: THREE.Scene, renderer: THREE.WebGLRenderer, camera: THREE.PerspectiveCamera) => {
    const { floorsData, currentFloorName } = resolveGeometryData();
    
    if (!floorsData || !floorsData[currentFloorName]) {
      console.log("RSP: No floor data available for single floor visualization");
      return;
    }

    // Transform data for Canvas3D compatibility
    const transformedFloorsData: Record<string, any> = {};
    transformedFloorsData[currentFloorName] = {
      ...floorsData[currentFloorName],
      hasClosedContour: (floorsData[currentFloorName] as any).lines?.length > 2,
      name: currentFloorName
    };

    // Generate geometry using Canvas3D shared function
    const geometryObjects = generateSharedFloorGeometry(transformedFloorsData, {
    ...
    console.log(`RSP: Added ${geometryObjects.length} single floor geometry objects using Canvas3D shared function`);
  };

  // Wall creation using shared Canvas3D geometry
  const createWalls = (
    scene: THREE.Scene,
    ...
    console.log("RoomSketchPro - Using lines data from resolveGeometryData:", currentLines);
    ...
    // Use single floor visualization with shared Canvas3D geometry
    createSingleFloorVisualization(scene, renderer, camera);
  };

  // Helper function for multifloor processing
  const processStairsFromCanvas3D = (stairData: any, floorName: string, stairIndex: number, multiFloorGroup: THREE.Group, floorGroups: Record<string, THREE.Group>) => {
    console.log(`Processing stair ${stairIndex} for floor ${floorName}`);
    // Simplified stair processing for now
  };
      ...
      const floorAirEntries = floorData.airEntries || [];
      floorAirEntries.forEach((entry: AirEntry) => {
