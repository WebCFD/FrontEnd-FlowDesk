...
export default function Canvas3D({
  ...
  // SURGICAL SOLUTION: Stable reference for isolation token
  const EDITING_ISOLATION_TOKEN = useMemo(() => ({}), []);
  ...
  const createFloorObjects = (
    ...
    if (floorData.furnitureItems && floorData.furnitureItems.length > 0) {
      floorData.furnitureItems.forEach((furnitureItem) => {
        ...
        if (!furnitureExists) {
        ...
        } else if (furnitureItem.type === 'rack' && sceneRef.current) {
          ...
          if (!hasDimGroup) {
            let existingModel: THREE.Object3D | null = null;
            sceneRef.current.traverse((child) => {
              if (child.userData?.furnitureId === furnitureItem.id && child.userData?.type === 'furniture') {
                existingModel = child;
              }
      ...
      if (intersects.length > 0) {
        ...
        if (floorData && floorData.airEntries) {
          ...
          if (foundIndex !== -1) {
            ...
            // Scene will automatically use stable EDITING_ISOLATION_TOKEN due to useMemo dependency change
  ...
  }, [finalFloors, currentFloor, ceilingHeight, floorDeckThickness, floorParameters]);
  ...
  return (
    <>
      ...
      <div ref={containerRef} className="w-full h-full relative">
        {/* Warning message overlay when no closed contour - non-blocking */}
        {!hasClosedContour && (
          <div className="absolute top-4 left-4 z-10 pointer-events-none">
            <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-r-lg shadow-lg max-w-sm bg-opacity-95">
              <div className="flex">
                ...
                <div className="ml-3">
                  <h3 className="text-sm font-medium text-yellow-800">No Room Contour</h3>
                  <p className="text-xs text-yellow-700 mt-1">
                    Create a closed room shape in 2D Editor to see the 3D model.
                  </p>
                </div>
              </div>
            </div>
